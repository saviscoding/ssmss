...
// 	const params = {
// 		TableName: "music_library" // my table name
// 	}
...
// 				alert("Error uploading file. Please try again.") // Display error message
// 			} else {
...
async function fetchAndDisplayURLs() {
	try {
		const response = await API.get("MyAPI", "/music-library")
...
async function uploadFile(event) {
	...
	try {
		const trackURL = await Storage.put(file.name, file)
		...
		// Add metadata to DynamoDB using Amplify's API module
		await API.post("MyAPI", "/music-library", dynamoDBParams)
